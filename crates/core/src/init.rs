...
pub struct DetectedProject {
    ...
    pub workspace_info: Option<String>, // e.g. "12 members across 5 directories"
}
...
pub fn validate_scan(root: &Path, scan_dirs: &[String], extensions: &HashSet<String>) -> usize {
...
pub fn generate_codescope_toml(detection: &DetectedProject) -> String {
    let mut out = String::new();
    out.push_str("# Generated by codescope init\n");
...
pub fn write_or_merge_mcp_json(root: &Path) -> Result<(), String> {
...
pub fn register_global_repo(root: &Path) -> Result<(), String> {
...
/// Build semantic search index for a project (pre-populates centralized cache).
#[cfg(feature = "semantic")]
pub fn build_semantic_index(root: &Path, model: Option<&str>) {
...
pub fn run_init(args: &[String]) -> i32 {
    ...
    eprintln!("codescope {} init", version);
    ...
    // Global repos.toml
    if global {
        if let Err(e) = register_global_repo(&root) {
            eprintln!("Error: {}", e);
            return 1;
        }
    }
    ...
    #[cfg(feature = "semantic")]
    if build_semantic {
        build_semantic_index(&root, None);
    }
